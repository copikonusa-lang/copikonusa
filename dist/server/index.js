...
var usersTable, productsTable, ordersTable, wishlistTable, reviewsTable, settingsTable, syncLogsTable, insertUserSchema, loginSchema, insertOrderSchema, insertReviewSchema, BRANCHES, ORDER_STATUS_MAP, CLIENT_STATUS_LABELS, ADMIN_STATUS_LABELS, PAYMENT_METHOD_LABELS, CATEGORIES;
var init_schema = __esm({
  "shared/schema.ts"() {
    ...
    productsTable = pgTable("products", {
    ...
    ordersTable = pgTable("orders", {
      ...
      amazonOrderIds: jsonb("amazon_order_ids").$type().default([]),
      amazonPurchaseStatus: text("amazon_purchase_status").default(""),
    ...
    wishlistTable = pgTable("wishlist", {
      id: text("id").primaryKey(),
      userId: text("user_id").notNull(),
    ...
    syncLogsTable = pgTable("sync_logs", {
      id: serial("id").primaryKey(),
    ...
    insertUserSchema = z.object({
      ...
      password: z.string().min(6, "M\xEDnimo 6 caracteres"),
      phone: z.string().min(7, "Tel\xE9fono requerido"),
  ...
  return {
    ...
    resolveId(source, importer) {
      if (/\.(png|jpg|jpeg|gif|webp)/.test(source) && importer) {
        const dir = path2.dirname(importer);
        let resolved = source;
        if (source.startsWith("@assets")) {
          resolved = source.replace("@assets", path2.resolve(import.meta.dirname, "attached_assets"));
        } else if (!path2.isAbsolute(source)) {
          resolved = path2.resolve(dir, source);
        }
...
var PLACEHOLDER_SVG, vite_config_default;
...
var PgStorage = class {
  ...
  async getCategories() {
    const rows = await this.db.select({ category: productsTable.category, count: count() }).from(productsTable).where(eq(productsTable.isActive, true)).groupBy(productsTable.category);
    const catNames = {
      tech: "Tecnolog\xEDa",
      phones: "Tel\xE9fonos",
      gaming: "Gaming",
  ...
  async getUserOrders(userId) {
    const rows = await this.db.select().from(ordersTable).where(eq(ordersTable.userId, userId)).orderBy(desc(ordersTable.createdAt));
    return rows;
  }
  async getAllOrders(filters) {
    ...
    const rows = await this.db.select().from(ordersTable).where(where).orderBy(desc(ordersTable.createdAt));
    return rows;
  }
  ...
  async addToWishlist(userId, productId) {
    const existing = await this.db.select().from(wishlistTable).where(and(eq(wishlistTable.userId, userId), eq(wishlistTable.productId, productId))).limit(1);
  ...
  async removeFromWishlist(userId, productId) {
    await this.db.delete(wishlistTable).where(and(eq(wishlistTable.userId, userId), eq(wishlistTable.productId, productId)));
  }
  // ===== REVIEWS =====
  async getProductReviews(productId) {
    const rows = await this.db.select().from(reviewsTable).where(eq(reviewsTable.productId, productId)).orderBy(desc(reviewsTable.createdAt));
    return rows;
  }
...
var MemStorage = class {
  ...
  async updateProduct(id, data) {
    const product2 = this.products.get(id);
    if (!product2) return void 0;
    const updated = { ...product2, ...data, id };
    if (data.basePrice !== void 0 || data.weight !== void 0) {
      const bp = data.basePrice ?? product2.basePrice;
      const w = data.weight ?? product2.weight;
      const shippingPerLb = parseFloat(await this.getSetting("shipping_per_lb") || "5.50");
...
async function getProductWeight(asin) {
  ...
  try {
    ...
    const product2 = data?.data?.amazonProduct;
    return {
      itemWeight: parseWeightToLbs(product2?.itemWeight),
      packageWeight: parseWeightToLbs(product2?.packageWeight),
      rawItem: product2?.itemWeight || null,
      rawPackage: product2?.packageWeight || null
    };
  } catch {
...
var LIGHT_BG = "#f4f4f4";
var WHITE = "#ffffff";
function baseTemplate(title, body, footerExtra) {
  return `<!DOCTYPE html>
<html lang="es">
...
<table width="600" cellpadding="0" cellspacing="0" style="background:${WHITE};border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08);">
  ...
  <tr><td style="background:${NAVY};padding:20px 24px;text-align:center;">
    ${footerExtra || ""}
    <p style="color:rgba(255,255,255,0.5);font-size:11px;margin:8px 0 0 0;">\xA9 ${(/* @__PURE__ */ new Date()).getFullYear()} CopikonUSA \u2014 Productos americanos al mejor precio</p>
...
async function registerRoutes(httpServer2, app2) {
  ...
  app2.post("/api/auth/login", async (req, res) => {
    try {
      ...
      const passwordMatch = isHashed ? await bcrypt2.compare(data.password, user.password) : user.password === data.password;
      if (!passwordMatch) {
        return res.status(401).json({ message: "Credenciales inv\xE1lidas" });
      }
      const token = generateToken();
  ...
  app2.get("/api/products/slug/:slug", async (req, res) => {
    const product2 = await storage.getProductBySlug(req.params.slug);
    if (!product2) return res.status(404).json({ message: "Producto no encontrado" });
    res.json(product2);
  });
  ...
  app2.get("/api/products/:id", async (req, res) => {
    const product2 = await storage.getProduct(+req.params.id);
    if (!product2) return res.status(404).json({ message: "Producto no encontrado" });
    res.json(product2);
  });
  ...
  app2.get("/api/search/amazon", async (req, res) => {
    try {
      ...
      }).filter((p) => p.weight <= 150).filter((p) => !isUnsendable(p.name)).filter((p) => checkShippingViability(p.amazonPrice, p.weight, "").viable).filter((p, i, arr) => arr.findIndex((x) => x.asin === p.asin) === i);
  ...
  app2.post("/api/search/import", async (req, res) => {
    try {
      ...
      const existingByAsin = await db2.select().from(pt).where(eq2(pt.amazonAsin, asin)).limit(1);
      ...
      const product2 = await pgStorage.createProduct({
        name,
        slug,
      ...
      res.json({ slug: product2.slug, id: product2.id });
    } catch (e) {
  ...
  app2.get("/api/variant/:asin", async (req, res) => {
    ...
    if (cached && Date.now() - cached.timestamp < 12 * 60 * 60 * 1e3) {
  ...
  app2.get("/api/products/:id/amazon-detail", async (req, res) => {
    try {
      const product2 = await storage.getProduct(+req.params.id);
      if (!product2) return res.status(404).json({ message: "Producto no encontrado" });
      const asin = product2.amazonAsin || product2.specs?.ASIN || "";
  ...
  app2.post("/api/admin/orders/:id/generate-cart", requireAdmin, async (req, res) => {
    ...
    if (storage instanceof PgStorage) {
      ...
      for (const item of order.items) {
        const [product2] = await db2.select().from(productsTable2).where(eq2(productsTable2.id, item.productId)).limit(1);
        if (product2) {
          estimatedAmazonCost += (product2.basePrice || 0) * item.quantity;
        }
      }
    }
  ...
  app2.patch("/api/admin/products/:id", requireAdmin, async (req, res) => {
    const product2 = await storage.updateProduct(+req.params.id, req.body);
    if (!product2) return res.status(404).json({ message: "Producto no encontrado" });
    res.json(product2);
  });
    ...
    (async () => {
      ...
      for (const search of todaySearches) {
        try {
          ...
          }).where(eq2(syncLogsTable3.id, log2.id));
  ...
  app2.post("/api/admin/sync/prices", requireAdmin, async (req, res) => {
    ...
    const { eq: eq2, sql: sqlTag } = await import("drizzle-orm");
    const stuckLogs = await db2.select().from(syncLogsTable3).where(eq2(syncLogsTable3.status, "running"));
    for (const stuck of stuckLogs) {
      ...
      if (Date.now() - startedAt > 30 * 60 * 1e3) {
        ...
        }).where(eq2(syncLogsTable3.id, stuck.id));
    ...
    (async () => {
      ...
      try {
        const allProducts = await db2.select().from(productsTable2).where(eq2(productsTable2.isActive, true)).orderBy(sqlTag`RANDOM()`).limit(500);
        const batchSize = 3;
        for (let i = 0; i < allProducts.length; i += batchSize) {
          ...
          await Promise.all(batch.map(async (product2) => {
            try {
              const asin = product2.specs?.ASIN || product2.amazonAsin;
              if (!asin) return;
              const unsendableReason = isUnsendable(product2.name);
              if (unsendableReason) {
                if (product2.isActive) {
                  await db2.update(productsTable2).set({ isActive: false }).where(eq2(productsTable2.id, product2.id));
                  deactivated++;
                  alerts.push({
                    type: "unsendable",
                  ...
                  console.log(`[SYNC] Deactivated unsendable: #${product2.id} "${product2.name.slice(0, 50)}" \u2014 ${unsendableReason}`);
                }
                processed++;
                return;
              }
              ...
              try {
                const weightData = await getProductWeight(asin);
                if (weightData.itemWeight || weightData.packageWeight) {
                  const realWeight = getBestWeight(weightData.itemWeight, weightData.packageWeight, weight, product2.name, product2.category);
                  ...
                  if (rawApiWeight > 150) {
                    if (product2.isActive) {
                      ...
                      console.log(`[SYNC] Deactivated overweight: #${product2.id} "${product2.name.slice(0, 50)}" \u2014 ${rawApiWeight} lbs`);
                    }
                    return;
                  }
                  const oldWeight = product2.weight || 1;
              ...
              const viability = checkShippingViability(newBasePrice, weight, product2.category);
              if (!viability.viable) {
                await db2.update(productsTable2).set({ isActive: false }).where(eq2(productsTable2.id, product2.id));
                deactivated++;
                alerts.push({
                ...
                console.log(`[SYNC] Deactivated shipping-prohibitive: #${product2.id} "${product2.name.slice(0, 50)}" \u2014 ratio ${viability.ratio}x`);
                return;
              }
              const newTotalPriceUsd = +(newBasePrice * 1.15 + weight * 5.5).toFixed(2);
              const oldTotalPrice = product2.totalPriceUsd;
              ...
              if (priceChange > 0.2) {
                ...
                alerts.push({
                  ...
                  weightChange: weightUpdated ? `${product2.weight} \u2192 ${weight} lbs` : null
                });
              }
              const updates = {
                basePrice: newBasePrice,
                ...
                isActive: true,
                rating: detail.rating || product2.rating,
                reviews: detail.ratingsTotal || product2.reviews
              };
              if (weightUpdated) {
              ...
              if (detail.mainImageUrl && detail.mainImageUrl !== product2.image) {
              ...
              await db2.update(productsTable2).set(updates).where(eq2(productsTable2.id, product2.id));
            } catch (e) {
              errors++;
              consecutiveErrors++;
            }
  ...
  app2.post("/api/admin/sync/translate", requireAdmin, async (req, res) => {
    ...
    (async () => {
      ...
      for (let i = 0; i < allProducts.length; i += BATCH) {
        ...
        try {
          for (const product2 of batch) {
            try {
              const translated_name = translateTitle(product2.name);
              if (translated_name !== product2.name) {
                await db2.update(productsTable2).set({ name: translated_name }).where(eq2(productsTable2.id, product2.id));
                translated++;
              }
            } catch {
              errors++;
  ...
  app2.post("/api/admin/sync/fix-weights", requireAdmin, async (req, res) => {
    ...
    const db2 = storage.db;
    const { productsTable: productsTable2, syncLogsTable: syncLogsTable3 } = await Promise.resolve().then(() => (init_schema(), schema_exports));
    const { eq: eq2, and: and2, sql: sql2, inArray } = await import("drizzle-orm");
    ...
    (async () => {
      try {
        ...
        const candidates = await db2.select({
          ...
          and2(
            eq2(productsTable2.isActive, true),
        ...
        const alerts = [];
        for (const product2 of candidates) {
          try {
            const unsendableReason = isUnsendable(product2.name);
            if (unsendableReason) {
              await db2.update(productsTable2).set({ isActive: false }).where(eq2(productsTable2.id, product2.id));
              deactivated++;
              alerts.push(`Deactivated #${product2.id}: ${unsendableReason} - ${product2.name.slice(0, 60)}`);
              continue;
            }
            const weightData = await getProductWeight(product2.amazonAsin);
            if (!weightData.itemWeight && !weightData.packageWeight) {
              const betterEstimate = estimateWeightByName(product2.name, product2.category);
              if (Math.abs(betterEstimate - product2.weight) > 0.5) {
                const newPrice2 = +(product2.basePrice * 1.15 + betterEstimate * 5.5).toFixed(2);
                await db2.update(productsTable2).set({
                  weight: betterEstimate,
                  totalPriceUsd: newPrice2,
                  specs: { ...product2.specs || {}, weightSource: "name_estimate" }
                }).where(eq2(productsTable2.id, product2.id));
                fixed++;
              } else {
                skipped++;
              }
            ...
            const realWeight = getBestWeight(
              ...
              estimateWeightByName(product2.name, product2.category),
              product2.name,
              product2.category
            );
            if (realWeight > 150) {
              await db2.update(productsTable2).set({ isActive: false }).where(eq2(productsTable2.id, product2.id));
              deactivated++;
              alerts.push(`Deactivated #${product2.id}: ${realWeight}lbs exceeds 150lb limit - ${product2.name.slice(0, 60)}`);
              continue;
            }
            const weightChange = Math.abs(realWeight - product2.weight);
            const weightChangePct = product2.weight > 0 ? weightChange / product2.weight * 100 : 999;
            const newPrice = +(product2.basePrice * 1.15 + realWeight * 5.5).toFixed(2);
            const oldPrice = +(product2.basePrice * 1.15 + product2.weight * 5.5).toFixed(2);
            await db2.update(productsTable2).set({
              weight: realWeight,
            ...
            if (weightChangePct > 100) {
              alerts.push(`#${product2.id}: weight ${product2.weight}\u2192${realWeight}lbs, price $${oldPrice}\u2192$${newPrice} (${product2.name.slice(0, 50)})`);
            }
            await new Promise((r) => setTimeout(r, 350));
          } catch (e) {
            errors++;
            console.error(`[WEIGHT FIX] Error on product ${product2.id}:`, e.message);
          }
        }
        await db2.update(syncLogsTable3).set({
          status: "completed",
  ...
  app2.post("/api/admin/sync/optimize-weights", requireAdmin, async (req, res) => {
    ...
    const db2 = storage.db;
    const { productsTable: productsTable2, syncLogsTable: syncLogsTable3 } = await Promise.resolve().then(() => (init_schema(), schema_exports));
    const { eq: eq2, sql: sqlTag, count: count2 } = await import("drizzle-orm");
      ...
      try {
        ...
        if (mode !== "weights-only") {
          ...
          for (const row of allActive.rows || allActive) {
            ...
            if (!viability.viable) {
              await db2.update(productsTable2).set({ isActive: false }).where(eq2(productsTable2.id, row.id));
              shippingFiltered++;
        ...
        if (mode !== "filter-only") {
          ...
          for (const product2 of rows) {
            if (Date.now() - startTime > 20 * 60 * 1e3) {
              console.log(`[OPTIMIZE] Time limit reached`);
              break;
            }
            try {
              const weightData = await getProductWeight(product2.amazon_asin);
              if (!weightData.itemWeight && !weightData.packageWeight) {
                const estimate = estimateWeightByName(product2.name, product2.category);
                const currentWeight = Number(product2.weight);
                if (Math.abs(estimate - currentWeight) > 0.5) {
                  const newPrice2 = +(Number(product2.base_price) * 1.15 + estimate * 5.5).toFixed(2);
                  const v2 = checkShippingViability(Number(product2.base_price), estimate, product2.category || "");
                  await db2.update(productsTable2).set({
                    weight: estimate,
                    totalPriceUsd: newPrice2,
                    isActive: v2.viable,
                    specs: { ...product2.specs || {}, weightSource: "smart_estimate", previousWeight: currentWeight }
                  }).where(eq2(productsTable2.id, product2.id));
                  weightFixed++;
              ...
              const oldWeight = Number(product2.weight);
              const newPrice = +(Number(product2.base_price) * 1.15 + realWeight * 5.5).toFixed(2);
              const v = checkShippingViability(Number(product2.base_price), realWeight, product2.category || "");
              await db2.update(productsTable2).set({
                weight: realWeight,
                totalPriceUsd: newPrice,
                isActive: v.viable,
              ...
              if (!v.viable) {
                shippingFiltered++;
                alerts.push(`[WEIGHT+SHIP] #${product2.id}: ${oldWeight}\u2192${realWeight}lbs, ratio ${v.ratio}x \u2192 deactivated - ${(product2.name || "").slice(0, 50)}`);
              } else if (Math.abs(realWeight - oldWeight) > 2) {
                alerts.push(`[WEIGHT] #${product2.id}: ${oldWeight}\u2192${realWeight}lbs, price $${Number(product2.base_price).toFixed(0)}\u2192$${newPrice} - ${(product2.name || "").slice(0, 50)}`);
              }
              await new Promise((r) => setTimeout(r, 350));
            } catch (e) {
              errors++;
        ...
        const countResult = await db2.select({ count: count2() }).from(productsTable2).where(eq2(productsTable2.isActive, true));
        ...
        await db2.update(syncLogsTable4).set({
          ...
          completedAt: (/* @__PURE__ */ new Date()).toISOString(),
          details: { error: e.message }
        }).where(eq2(syncLogsTable4.id, log2.id));
      }
    })();
  });
  app2.post("/api/admin/sync/cleanup", requireAdmin, async (req, res) => {
    ...
    const db2 = storage.db;
    const { productsTable: productsTable2, syncLogsTable: syncLogsTable3 } = await Promise.resolve().then(() => (init_schema(), schema_exports));
    const { eq: eq2, and: and2, sql: sqlTag, count: count2 } = await import("drizzle-orm");
    const [log2] = await db2.insert(syncLogsTable3).values({
        ...
        for (const row of dupeRows.rows || dupeRows) {
          ...
          if (ids.length > 1) {
            for (const id of ids.slice(1)) {
              await db2.update(productsTable2).set({ isActive: false }).where(eq2(productsTable2.id, id));
              deactivated++;
            }
...
`;
  ...
  app2.get("/p/:slug", async (req, res) => {
    try {
      const product2 = await storage.getProductBySlug(req.params.slug);
      if (!product2) return res.redirect("/#/catalogo");
      const priceUsd = product2.totalPriceUsd.toFixed(2);
      const imgUrl = product2.image?.startsWith("http") ? `${DOMAIN}/api/img?url=${encodeURIComponent(product2.image)}` : `${DOMAIN}${product2.image}`;
      const catSeo = CATEGORY_SEO[product2.category];
      const catName = catSeo?.h1 || product2.category;
      const jsonLd = {
        "@context": "https://schema.org",
        "@type": "Product",
        "name": product2.name,
        "description": product2.description || `Compra ${product2.name} en CopikonUSA con env\xEDo a\xE9reo a Venezuela. Pago en bol\xEDvares.`,
        "image": imgUrl,
        "brand": {
          "@type": "Brand",
          "name": "CopikonUSA"
        ...
        "offers": {
          "@type": "Offer",
          "url": `${DOMAIN}/#/producto/${product2.slug}`,
          "priceCurrency": "USD",
          "price": priceUsd,
          "availability": product2.isActive ? "https://schema.org/InStock" : "https://schema.org/OutOfStock",
          "seller": {
            "@type": "Organization",
            "name": "CopikonUSA"
          },
        ...
        ...product2.rating && product2.reviews ? {
          "aggregateRating": {
            "@type": "AggregateRating",
            "ratingValue": product2.rating.toFixed(1),
            "reviewCount": product2.reviews
          }
        } : {},
        "category": catName
      };
...
<head>
  ...
  <title>${escapeHtml(product2.name)} \u2014 CopikonUSA</title>
  <meta name="description" content="Compra ${escapeHtml(product2.name)} por $${priceUsd} USD con env\xEDo a\xE9reo desde USA a Venezuela. Pago en bol\xEDvares. ${catName} en CopikonUSA.">
  <meta property="og:title" content="${escapeHtml(product2.name)} \u2014 CopikonUSA">
  <meta property="og:description" content="$${priceUsd} USD \u2014 Env\xEDo a\xE9reo USA a Venezuela. Pago en bol\xEDvares.">
  <meta property="og:image" content="${imgUrl}">
  <meta property="og:url" content="${DOMAIN}/#/producto/${product2.slug}">
  <meta property="og:type" content="product">
  <meta property="product:price:amount" content="${priceUsd}">
  <meta property="product:price:currency" content="USD">
  <link rel="canonical" href="${DOMAIN}/#/producto/${product2.slug}">
  <script type="application/ld+json">${JSON.stringify(jsonLd)}</script>
</head>
<body>
  <h1>${escapeHtml(product2.name)}</h1>
  <p>Precio: $${priceUsd} USD</p>
  <p>${escapeHtml(product2.description || "")}</p>
  <p>Categor\xEDa: ${escapeHtml(catName)}</p>
  <a href="${DOMAIN}/#/producto/${product2.slug}">Ver producto en CopikonUSA</a>
  <script>window.location.href = "${DOMAIN}/#/producto/${product2.slug}";</script>
</body>
</html>`);
    } catch (err) {
